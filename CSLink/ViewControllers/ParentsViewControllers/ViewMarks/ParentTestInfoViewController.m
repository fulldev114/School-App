...
@interface ParentTestInfoViewController ()
{
    NSDictionary *testDetail;
    BOOL checkBoxSelected;
    NSString *isImagWith;
}
@end

@implementation ParentTestInfoViewController

- (id)initWithNibName:(NSString *)nibNameOrNil bundle:(NSBundle *)nibBundleOrNil andDetail:(id)obj {
    
    self = [super initWithNibName:nibNameOrNil bundle:nibBundleOrNil];
    
    testDetail = (NSDictionary *) obj;
...
-(void)setUpUi{
    ...
    lblStudantName.text = [[testDetail valueForKey:@"studant_Detail"] valueForKey:@"child_name"];
    
    lblClassName.font = FONT_16_BOLD;
    lblClassName.textColor = TEXT_COLOR_WHITE;
    lblClassName.text = [testDetail valueForKey:@"class_name"];
  ...
  //  [btnWithImage setBackgroundImage:[UIImage imageNamed:@"icon_check.png"] forState:UIControlStateHighlighted];
    isImagWith = @"no";
...
- (IBAction)btnDownloadPress:(id)sender {
    ...
    NSMutableArray *arrSelectedSubject = [NSMutableArray arrayWithObject:dicMarksDetail];
    
    [appDelegate generateAndUploadPdf:[testDetail valueForKey:@"studant_Detail"] andSubjectDetail:arrSelectedSubject withImg:isImagWith isIndividual:YES];
}
