...
- (void) setupBadgeViewWithString:(NSString *)string
{
    ...
    if (IS_IPAD) {
        ...
        badgeSize = [badgeLabel sizeThatFits:CGSizeMake(320, FLT_MAX)];
        badgeSize.width = badgeSize.width < 20 ? 35 : badgeSize.width + 5;
    }
    else {
        [badgeLabel setFont:[UIFont systemFontOfSize:13]];
        badgeSize = [badgeLabel sizeThatFits:CGSizeMake(320, FLT_MAX)];
    ...
    [badgeLabel setFrame:CGRectMake(horizontal, vertical, badgeSize.width,  badgeSize.width)];
...
- (void) setupBadgeStyle
{
    ...
    badgeLabel.layer.cornerRadius = badgeLabel.bounds.size.width / 2; //badgeLabel.bounds.size.width > 25 ? 8 : badgeLabel.bounds.size.width / 2;
}
