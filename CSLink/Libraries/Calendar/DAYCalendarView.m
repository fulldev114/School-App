...
@interface DAYCalendarView () {
    ...
    NSMutableArray *_eventsInVisibleMonth;
}
...
- (void)commonInit {
	...
	self.weekdayHeaderTextColor = TEXT_COLOR_CYNA;
    //self.weekdayHeaderWeekendTextColor = [UIColor colorWithRed:0.75 green:0.25 blue:0.25 alpha:1];
	
	self.weekdayHeaderWeekendTextColor = TEXT_COLOR_CYNA;
...
//	self.selectedIndicatorColor = [UIColor clearColor];
	...
	self.todayIndicatorColor = TEXT_COLOR_CYNA;
...
- (void)configureWeekdayHeaderView {
    ...
    [self.weekHeaderView.arrangedSubviews enumerateObjectsUsingBlock:^(__kindof UIView * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
		...
		weekdayLabel.font = FONT_16_BOLD;
...
- (void)configureComponentView:(DAYComponentView *)view withDay:(NSUInteger)day month:(NSUInteger)month year:(NSUInteger)year {
    ...
    if (self.showUserEvents) {
        [self->_eventsInVisibleMonth enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            ...
            if ([[DAYUtils dateFromDateComponents:comps] isEqualToDate:[dateFormat dateFromString:[event valueForKey:@"date"]]]) {
    ...
    if (self->_visibleMonth == month && self.boldPrimaryComponentText) {
		...
		view.textLabel.font = FONT_16_BOLD;
		view.alpha = 1.0;
    }
    else {
        //view.textLabel.font = [UIFont systemFontOfSize:16];
		view.textLabel.font = FONT_16_REGULER;
		view.alpha = 0.0;
    }
...
{
    if (_eventsInVisibleMonth != nil)
        [_eventsInVisibleMonth removeAllObjects];
    
    _eventsInVisibleMonth = [NSMutableArray arrayWithArray:events];
...
- (void)getCalendarData
{
    ...
    if (self.showUserEvents) {
        ...
        NSDateFormatter *dateFormat = [[NSDateFormatter alloc] init];
        [dateFormat setDateFormat:@"yyyy-MM-dd"];
        NSString *fromDate = [dateFormat stringFromDate:[DAYUtils dateFromDateComponents:startComps]];
        NSString *toDate = [dateFormat stringFromDate:[DAYUtils dateFromDateComponents:endComps]];
        
        [self.delegate updateDateRegion:fromDate toDate:toDate];
    }
    else {
