...
+ (NSString *)stringOfWeekdayInEnglish:(NSUInteger)weekday {
    ...
    dispatch_once(&onceToken, ^{
		...
		Strings = @[@"SU", @"MO", @"TU", @"WE", @"TH", @"FR", @"SA"];
    });
    
    return Strings[weekday - 1];
}

+ (NSString *)fullStringOfWeekdayInEnglish:(NSUInteger)weekday {
    NSAssert(weekday >= 1 && weekday <= 7, @"Invalid weekday: %lu", (unsigned long) weekday);
    static NSArray *Strings;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
...
+ (NSDateComponents *)dateComponentsForWeekDayFromDate:(NSDate *)date {
    return [[self localCalendar] components:NSCalendarUnitWeekday fromDate:date];
}
