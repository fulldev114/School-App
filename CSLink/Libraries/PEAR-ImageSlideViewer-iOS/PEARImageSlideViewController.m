...
- (void)setZoomViewWithImageLists:(NSArray *)imageLists
{
    ...
    for (NSInteger i =0; i < imageLists.count; i++)
    {
        ...
        if ([imageLists[i] isKindOfClass:[UIImage class]])
            zoomView.imageView.image = imageLists[i];
        else if ([imageLists[i] isKindOfClass:[NSURL class]])
            [zoomView.imageView setImageWithURL:imageLists[i] placeholderImage:[UIImage imageNamed:@"gray-image"]];
